...
public:
	...
	void write_login_error(std::string_view msg);
	...
	void write_add_marker(const Position& pos, uint8_t mark_type, std::string_view desc);
	...
	void write_channel(uint16_t id, std::string_view name, const std::map<uint32_t, Player*>* users,
	                   const std::map<uint32_t, const Player*>* invited_users);
	void write_channel_event(uint16_t id, std::string_view player_name, ChannelEvent_t event);
	void write_channel_message(std::string_view author, std::string_view text, SpeakClasses type, uint16_t channel);
	...
	void write_create_private_channel(uint16_t channel_id, std::string_view channel_name);
	...
	void write_creature_say(const Creature* creature, SpeakClasses type, std::string_view text,
	...
	void write_fyi_box(std::string_view message);
	void write_house_window(uint32_t id, std::string_view text);
	...
	void write_open_private_channel(std::string_view receiver);
	void write_outfit_window();
	void write_pending_state_entered();
	void write_ping();
	void write_ping_back();
	void write_podium_window(const Item* item);
	void write_private_message(const Player* speaker, SpeakClasses type, std::string_view text);
	...
	void write_text_window(uint32_t id, uint32_t item_id, std::string_view text);
	void write_to_channel(const Creature* creature, SpeakClasses type, std::string_view text, uint16_t channel_id);
	...
	void write_updated_vip_status(uint32_t guid, VipStatus_t status);
	void write_use_item_cooldown(uint32_t time);
	void write_vip(uint32_t guid, std::string_view name, std::string_view description, uint32_t icon, bool notify,
	               VipStatus_t status);
	void write_vip_entries();
	void write_items(std::map<uint32_t, uint32_t> inventory);

private:
	virtual Player* get_player() const = 0;

	virtual void enqueue(std::span<uint8_t> span) = 0;
